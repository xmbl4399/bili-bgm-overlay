...
| 顶栏（自有） | `● Bangumi` + 二级分类 tab `TV / WEB / OVA / 剧场版 / 日剧 / 欧美剧 / 华语剧 / 电影`（横向可滚，胶囊选中）+ 主题按钮 `◐/☀/☾` + 设置 ⚙ |
...
|---|---|---|---|
| **r100**（默认） | 6.2 KB | **0.43 MB** | 1× 屏够用；2× 屏会糊 |
| r200 | 20.5 KB | 1.3 MB | 2× 屏偏紧 |
...
r100 明显发虚、r400 起海报小字清晰可读 ⇒ 高档位屏建议选 `r400`，1× 屏用默认 `r100` 即可。
...
| `images.common`（经 `pickCover()`：common → medium → large → small） | 封面 | 卡片封面，两条通路统一以 **r/400** 为基准；**实际宽度在渲染时按「封面质量」改写**（`withCoverWidth()`，默认 r100，见 [§8](docs/bangumi-list-api-facts.md)） |
